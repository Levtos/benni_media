...
export function RulesPage({ matrix, apply = {}, state = {}, hass, onMatrix }: { matrix?: MatrixData; apply?: ApplyData; state?: StateData; hass: HassLike; onMatrix: (matrix: MatrixData) => void }) {
  ...
  return <div className={`rules-page ${busy ? "busy" : ""}`}>
    <div className="rules-tabs">
      ...
      <span />
      <button className="button danger" title="Löscht ausschließlich Matrix-Overrides für Basis, Szenario und Activity. Policy-Skalare und Apply-Options bleiben unverändert." onClick={async () => { if (!window.confirm("Globale Matrix zurücksetzen? Betroffen sind ausschließlich Tagesphasen-Basis, Szenario-Offsets und Activity-Offsets. Fenster, Gaming-Skalare, Caps und Apply-Optionen bleiben unverändert.")) return; onMatrix(await resetMatrix(hass)); }}><Icon.RefreshCw size={17} />Globale Matrix zurücksetzen</button>
    </div>
