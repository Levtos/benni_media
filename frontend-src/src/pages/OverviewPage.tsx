...
export function OverviewPage({ data, hass, onChanged }: { data?: OverviewData; hass: HassLike; onChanged: () => void }) {
  ...
  const radio = apply.radio?.defaults || [];
  return <>
    ...
    <div className="overview-hero">
      <div className="session-grid">
        {music ? <ActiveMediaCard kind="music" eyebrow="Musik" title={music.title || "Unbekannter Titel"} subtitle={music.artist} detail="Musik läuft über die HomePods" device="HomePods" volume={music.volume ?? devices.homepods?.volume} artwork={music.artwork_url} controls={<SegmentedButton disabled title="Play/Pause benötigt einen additiven Backend-Contract" icon={Icon.Pause}>Pause</SegmentedButton>} /> : <Card><EmptyState icon={Icon.Music2} title="Keine Musik-Session" text="Sobald die HomePods spielen, erscheint die Session hier." /></Card>}
        {isGaming ? <ActiveMediaCard kind="gaming" eyebrow="Gaming" title={gameTitle} subtitle={titleCase(state.gaming_platform || data?.gaming_platform || data?.gaming_source)} detail={policy.is_grind ? "Grind-Modus aktiv" : titleCase(state.subcontext)} device={devices.denon?.active ? "Denon AVR-X1400H" : "Gaming-Audio"} volume={policy.volume_target_denon ?? data?.targets?.denon_volume} artwork={gameDevice?.artwork_url} /> : <Card><EmptyState icon={Icon.Gamepad2} title="Keine Gaming-Session" text="PlayStation- oder PC-Gaming wird parallel zur Musik eingeblendet." /></Card>}
      </div>
    ...
    <div className="dashboard-grid lower">
      <Card className="quick-control" title="Schnellsteuerung">
        ...
        <h3>Radio-Schnellwahl</h3><div className="radio-row">{radio.map((station) => <button key={station.uri} onClick={() => void run("apply", "play_radio", { media_id: station.uri })}><Icon.Radio size={17} /><span>{station.name}</span></button>)}{!radio.length && <EmptyState compact title="Keine Radio-Favoriten" text="Media Apply liefert keine Default-Sender." />}</div>
      </Card>
